...
def load_numpy_da_file(f, **kwargs):
    ...
    '''
    if labels:
        data = numpy.loadtxt(d2l, dtype=numpy.float, delimiter=' ', skiprows=1)
    else:
        data = numpy.loadtxt(d2l, dtype=numpy.float, delimiter=' ', skiprows=0)
    cnt = 0
    for row in data:
        lt = len(row)
        if lt != 63:
    ...
    #print('')
    #print('')
    #print('len of dict is now {:d}'.format(len(bd_data)))
...
def get_attrib_array(np_data, **kwargs):
    ...
    #for i in range(1, col_s):
    #    attrib_array = numpy.vstack((attrib_array, np_data[:,i]))

    #for i in range(col_s):
    #    print(attrib_array.tolist()[i])
    #print('')

    #for i in range(col_s):
    #    print(data_trns.tolist()[i])
    #print('')
    #print('')
    #print('')
    #print(attrib_array.tolist()[0])
    #print('')
    #print(data_trns.tolist()[0])
    #print('')
    #print(attrib_array.tolist()[col_s-1])
    #print('')
    #print(data_trns.tolist()[col_s-1])
...
def perform_imputation(data, reg_data,  bad_data, **kwargs):
    ...
    if imputation == 'average':
        ...
        return numpy.array(reg_data, dtype=numpy.float)
...
def process_csv_to_json(csv_file_name, json_file_name, **kwargs):

    Headr = kwargs.get('head', True)
    label_str = label_start
    ...
    for row in dreader:
        ...
        for idx in range(2, len(headers)):
            ...
            if scp != -1 or cmma != -1 or dlr != -1:
                row[header] = fix_str_to_num(val)
            #if idx in to_fix:
            #    print(val, idx)
            #    idx_to_fix.append(idx)
        #print('')
        #cnt += 1
        #if cnt > 0:
        #    break
    ...
    #print('indexs are:')
    #print('there are {:d} indices to fix'.format(len(to_fix)))
    #print(to_fix)
    csv_file.close()
    json_file.close()
...
def process_json_to_data_array(json_file_name, s_names, headers, **kwargs):
    ...
    #print('')
    #print('The length of the array is {:d}'.format(number_of_schools))
    #print('')
    ...
    for i in range(0, 57):
        ...
        #print('entry {:d}  name is is {:s}'.format(i, file_text['Name']))
        c = 0
        attrib_list = list()
        for idx in range(len(headers)):
            if idx == 0:
                ...
                continue
            entry = headers[idx]
            #print('The entry {:d} is {:s}'.format(c, entry))
            c += 1
            val = file_text[entry]
            val = val.strip('$')
            ...
            '''
            comma_cnt = val.count(',')
            #print('there are {:d} commas '.format(comma_cnt))

            spce_cnt = val.count(' ')
            #print('there are {:d} commas '.format(spce_cnt))
            ...
            if entry not in stop_colls:
                ...
                if entry == 'Total Faculty':
                    #print('1found it at {:d} val is {:s} entry {:s}'.format(idx, val, entry))
                    val = float(val)
                    #attrib_list.append(val)
                    #print(val)
                elif val.isnumeric():
                    val = float(val)
                    #  elif entry == 'Total Faculty':
                    #      print('found it at {:d}'.format(c))
                    #      val = float(val)
                    #      print(val)
                else:
    ...
    #print('the number of school names is {:d}'.format(num_schools))
    #print('the 2014 med school options are ')
    #print(med2014dict)
    #print('the vet school options are ')
    #print(vetschool)
    return data_array
# ---------------------------------------------------------------------------------------------------------------------
...
#print('')
#print('')
#print('header at 29 is {:s}'.format(headers[29]))
#print('header at 64 is {:s}'.format(headers[64]))
#print(len(headers))

#cnt = 0
#for x in headers:
#    print('{:d}: {:s}'.format(cnt, x))
#    cnt +=1
...
a_labels = headers[2:4] + headers[5:]

print('Attriburte Labels:')
print(len(a_labels))
print(a_labels)
print('')
